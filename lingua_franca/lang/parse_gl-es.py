...
from lingua_franca.lang.format_gl-es import pronounce_number_gl-es
from lingua_franca.lang.parse_common import *
from lingua_franca.lang.common_data_gl-es import _ARTICLES_GL-ES, _STRING_NUM_GL-ES


def is_fractional_gl-es(input_str, short_scale=True):
    """
...
def extract_number_gl-es(text, short_scale=True, ordinals=False):
    """
    ...
    while count < len(aWords):
        ...
        if word in _STRING_NUM_GL-ES:
            val = _STRING_NUM_GL-ES[word]
        ...
        if next_word in ands:
            ...
            afterAndVal = extract_number_gl-es(newText[:-1])
            if afterAndVal:
        ...
        if next_word in decimals:
            ...
            for word in newWords:
                if word == "cero" or word == "0":
                    ...
                    break
            afterDotVal = str(extract_number_gl-es(newText[:-1]))
...
def _gl-es_number_parse(words, i):
    ...
    def gl-es_number_1_99(i):
        r1 = es_number_word(i, 1, 29)
        if r1:
            return r1

        r1 = gl-es_number_word(i, 30, 90)
        if r1:
            v1, i1 = r1
            r2 = es_cte(i1, "y")
            if r2:
                i2 = r2[1]
                r3 = gl-es_number_word(i2, 1, 9)
                if r3:
                    v3, i3 = r3
                    return v1 + v3, i3
            return r1
    ...
    def gl-es_number_1_999(i):
        # [2-9]centos [1-99]?
        r1 = gl-es_number_word(i, 100, 900)
        if r1:
            v1, i1 = r1
            r2 = gl-es_number_1_99(i1)
            if r2:
                v2, i2 = r2
                return v1 + v2, i2
            else:
    ...
    def gl-es_number(i):
        # check for cero
        r1 = gl-es_number_word(i, 0, 0)
        if r1:
            return r1

        # check for [1-999] (mil [0-999])?
        r1 = gl-es_number_1_999(i)
        if r1:
            v1, i1 = r1
            r2 = es_cte(i1, "mil")
            if r2:
                i2 = r2[1]
                r3 = gl-es_number_1_999(i2)
                if r3:
                    v3, i3 = r3
    ...
    return gl-es_number(i)


def extract_numbers_gl-es(text, short_scale=True, ordinals=False):
    """
    ...
    """
    return extract_numbers_generic(text, pronounce_number_gl-es,
                                   extract_number_gl-es, short_scale=short_scale,
                                   ordinals=ordinals)


def normalize_gl-es(text, remove_articles=True):
    """ Galician string normalization """
    # TODO return GalicianNormalizer().normalize(text, remove_articles)
    ...
    while i < len(words):
        ...
        if remove_articles and word in _ARTICLES_GL-ES:
            i += 1
            continue

        # Convert numbers into digits
        r = _gl-es_number_parse(words, i)
        if r:
            v, i = r
            normalized += " " + str(v)
            continue
...
def extract_datetime_gl-es(text, anchorDate=None, default_time=None):
    def clean_string(s):
        # cleans the input string of unneeded punctuation and capitalization
        # among other things
        symbols = [".", ",", ";", "?", "!", "º", "ª"]
        noise_words = ["entre", "a", "do", "ao", "o", "de",
                       "para", "unha", "calquera", "a",
                       "e'", "esta", "este"]
        ...
        synonyms = {"mañá": ["amañecer", "cedo", "moi cedo"],
                    "tarde": ["media tarde", "atardecer"],
                    "noite": ["anoitecer", "tarde"]}
        for syn in synonyms:
            for word in synonyms[syn]:
                s = s.replace(" " + word + " ", " " + syn + " ")
        # relevant plurals, cant just extract all s in pt
        wordlist = ["mañás", "tardes", "noites", "días", "semanas",
                    "anos", "minutos", "segundos", "as", "os", "seguintes",
    ...
    timeQualifiersList = ['mañá', 'tarde', 'noite']
    time_indicators = ["en", "a", "ao", "por", "pasados",
                       "pasadas", "día", "hora"]
    days = ['luns', 'martes', 'mércores',
            'xoves', 'vernes', 'sábado', 'domingo']
    ...
    monthsShort = ['xan', 'feb', 'mar', 'abr', 'mai', 'xuñ', 'xul', 'ago',
                   'set', 'out', 'nov', 'dec']
    nexts = ["seguinte", "próximo", "próxima"]
    suffix_nexts = ["seguintes", "subsecuentes"]
    lasts = ["último", "última"]
    suffix_lasts = ["pasada", "pasado", "anterior", "antes"]
    nxts = ["despois", "seguinte", "próximo", "próxima"]
    prevs = ["antes", "previa", "previo", "anterior"]
    froms = ["desde", "en", "para", "despois de", "por", "próximo",
             "próxima", "de"]
    thises = ["este", "esta"]
    froms += thises
    ...
    for idx, word in enumerate(words):
        ...
        # parse today, tomorrow, yesterday
            ...
            dayOffset = 0
            used += 1
        elif word == "mañá" and not fromFlag:
            dayOffset = 1
            used += 1
        ...
        elif (word == "antonte" or
              (word == "antes" and wordNext == "de onte")) and not fromFlag:
            dayOffset -= 2
            used += 1
            if wordNext == "onte":
                used += 1
        elif word == "antes" and wordNext == "antes" and wordNextNext == \
                "de onte" and not fromFlag:
            dayOffset -= 3
            used += 3
        elif word == "antes de antonte" and not fromFlag:
            dayOffset -= 3
            used += 1
        # day after tomorrow
        elif word == "pasado" and wordNext == "mañá" and not fromFlag:
            dayOffset += 2
            used = 2
        # day before yesterday
        elif word == "antes" and wordNext == "de antonte" and not fromFlag:
            dayOffset -= 2
            used = 2
        ...
        validFollowups.append("mañá")
        validFollowups.append("onte")
        validFollowups.append("antonte")
        validFollowups.append("agora")
        validFollowups.append("xa")
        validFollowups.append("antes")

        # TODO debug word "despois" that one is failing for some reason
        if word in froms and wordNext in validFollowups:

            if not (wordNext == "mañá" and wordNext == "onte") and not (
                    word == "pasado" or word == "antes"):
                used = 2
                fromFlag = True
            if wordNext == "mañá" and word != "pasado":
                dayOffset += 1
            elif wordNext == "onte":
                dayOffset -= 1
            elif wordNext == "antonte":
                dayOffset -= 2
            elif wordNext == "antes" and wordNextNext == "de onte":
                dayOffset -= 2
            elif (wordNext == "antes" and wordNext == "antes" and
                  wordNextNextNext == "de onte"):
                dayOffset -= 3
    ...
    for idx, word in enumerate(words):
        ...
        if word == "medio" and wordNext == "día":
            hrAbs = 12
            used += 2
        elif word == "media" and wordNext == "noite":
            hrAbs = 0
            used += 2
        elif word == "mañá":
            if not hrAbs:
            ...
            if not hrAbs:
                hrAbs = 20
            used += 2
        elif word == "media" and wordNext == "mañá":
            if not hrAbs:
                hrAbs = 10
            used += 2
        # elif word == "a" and wordNext == "tardecer":
        #     if not hrAbs:
        #         hrAbs = 19
        #     used += 2
        # elif word == "a" and wordNext == "mañecer":
        #     if not hrAbs:
        #         hrAbs = 11
        #     used += 2
        elif word == "madrugada":
            ...
            if ':' in word:
                ...
                if remainder == "":
                    ...
                    if nextWord == "am" or nextWord == "pm":
                        remainder = nextWord
                        used += 1
                    elif wordNext == "mañá" or wordNext == "madrugada":
                        remainder = "am"
                        used += 1
                    elif wordNext == "tarde":
                        remainder = "pm"
                        ...
                        if 0 < int(word[0]) < 6:
                            ...
                            remainder = "pm"
                        used += 1
                    elif wordNext in thises and wordNextNext == "mañá":
                        remainder = "am"
                        used = 2
...
def get_gender_gl-es(word, context=""):
    ...
    for idx, w in enumerate(words):
        if w == word and idx != 0:
            ...
            gender = get_gender_gl-es(previous)
            break
    if not gender:
        if word[-1] == "a":
            gender = "f"
...
class GalicianNormalizer(Normalizer):
